 ...
 * @author Intektor
 */
public class GuiComponentFoundGames extends GuiFrame {
    ...
    public GuiComponentFoundGames(int x, int y, int width, int height) {
    ...
    protected void postDrawBody(float drawX, float drawY, OrthographicCamera camera, ShapeRenderer sR, SpriteBatch sB, float partialTicks, int mouseX, int mouseY) {
        ...
        sB.end();
        Gdx.gl.glScissor(GuiUtils.unscaleScreenCoordX(GuiComponentFoundGames.this.x), GuiUtils.unscaleScreenCoordY(GuiComponentFoundGames.this.y),
                GuiUtils.unscaleScreenCoordX(GuiComponentFoundGames.this.width), GuiUtils.unscaleScreenCoordY(GuiComponentFoundGames.this.height - topBarHeight - 100));
    ...
    private void setSelected(HostingType hostingType) {
        ...
        serverInfos.clear();
        if (hostingType == HostingType.LAN) {
